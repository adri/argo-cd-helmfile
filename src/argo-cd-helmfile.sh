...
if [[ "${HELMFILE_BINARY}" ]]; then
  ...
  if [[ $(which helmfile) ]]; then
    ...
    output=$(helm plugin install https://github.com/futuresimple/helm-secrets)
    if [ $? -ne 0 ]; then
        echo ":x: Failed installing plugin"
        echo "$output"
        exit 1;
    fi
    output1=$(helm2 init --client-only ; helm plugin install https://github.com/futuresimple/helm-secrets)
    if [ $? -ne 0 ]; then
